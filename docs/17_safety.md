...
- **A `.fw` file that fails local validation** (bad footer CRC, wrong
  ...
  (`docs/13_experiments.md` Experiment 5, `docs/14_failed_attempts.md`) —
...
as any other flash (`docs/15_firmware_patching.md` §15.1). This is exactly
...
output (`docs/05_firmware_layout.md` §5.3) shows a dedicated `bootloader`
...
**not a tested or confirmed recovery path**. See `docs/18_future_work.md`
...
(`docs/14_failed_attempts.md`). The working solution was `pkexec`
...
- The exact byte offsets used in `scripts/patch_led_black.py` (the
...
- `scripts/patch_led_firmware.py` (the config-value patch) locates its
  target via the config table structure format
  (`docs/06_firmware_symbols.md` §6.2), which is somewhat more likely to
...
- Both scripts should be treated as validated **only** for
  ...
  `hashes/firmware_hashes.txt`) until independently re-verified against
  any other build.
...
1. Do not panic-flash repeatedly — if a flash fails, understand *why*
   before retrying (see `docs/14_failed_attempts.md` and
   `docs/13_experiments.md` for the failure modes already characterized).
2. Check `docs/11_hid_commands.md` (`info`, `flash_info`, `battery`) for
