...
(`../docs/14_failed_attempts.md` "Environment/tooling dead ends").

| Tool | Version used | Purpose | Setup doc |
|---|---|---|---|
| radare2 | 5.9.8 | x86-64 disassembly of `lighthouse_watchman_update` (Valve's official update tool), and early-stage ARM firmware analysis | `radare2_setup.md` |
| Ghidra | 12.1.2 | Full ARM Cortex-M4/Thumb-2 disassembly and decompilation of the controller's application firmware | `ghidra_setup.md` |
| capstone (Python library) | 5.0.9 | Lightweight scriptable Thumb-2 disassembly, used before Ghidra was set up (`../scripts/disasm_config.py`) | installed via `pip install --user capstone`, no special setup needed |
